              ...
              if String.equal next_seg "" then
                ...
                let next_parent =
                  Some
                    (match parent with
                    | None ->
                        if String.equal seg "" then Fpath.v "/" else Fpath.v seg
                ...
                in
                ( res,
                  Some { name = root.name; contents = Directory updated_files }
                )
            else (None, Some root))
  in
...
let write_file state ~path ~contents =
  let _, state =
    file_system_action state path ~create:true ~f:(fun _ f ->
                  ...
                  (List.fold_right files ~init:[] ~f:(fun file acc ->
                       ...
                       if not (filter full_path) then acc
                         ...
                         if not recurse then full_path :: acc
                           ...
                           | Directory files ->
                               files_of_directory' (Some full_path) files @ acc))
              in
