...
module type Interface = sig
  ...
  val init_state : t
  val file_exists : t -> Fpath.t -> [ `Yes | `No | `Unknown ]
  val is_dir : t -> Fpath.t -> [ `Yes | `No | `Unknown ]
  val read_file : t -> Fpath.t -> (string, string) Either.t
  val write_file : t -> path:Fpath.t -> contents:string -> t
...
module System : Interface = struct
  ...
  let write_file _ ~path ~contents =
